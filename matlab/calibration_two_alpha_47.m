...
n_L  = [1137265 1097857 1000711]; % from 各世代平均生育總數
n_H  = [ 144652  209308  316335]; % from 各世代平均生育總數
...
[x,fval_m]=fsolve(@(x)myfun_2a_c(x,pb_m(a),pe_m(b),am(c),c_m(d),k0(e),k1(e),k2(e),N_L(i),N_H(i),rho,alpha_L,alpha_H,gamma,beta(i),A(i),pi(i)),xi,options);
 %   pb_m(i)      = pb_m(a)*(1-Q) ;
    pe_m(i)      = pe_m(b) ;
    %am_m(i)      = (am(c)-(N_L(i)*(x(4)*x(1)+(1-x(4))*x(2))+N_H(i)*x(3))*pb_m(i)*Q) ; %p1
  %  am_m(i)      = (am(c)-(N_L(i)*(x(4)*x(1)+(1-x(4))*x(2)))*pb_m(i)*Q) ; %p2
    c_m(i)       = c_m(d) ;
    k0(i)        = k0(e) ;
    k1(i)        = k1(e) ;
    k2(i)        = k2(e) ;
