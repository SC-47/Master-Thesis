function F = myfun_2a_c_p3 (x,pb,pe,am,c_,k0,k1,k2,N_L,N_H,rho,alpha_L,alpha_H,gamma,beta,A,pi,Q) 
...
F = [ (1+gamma+alpha_L)*pb*(1-Q)*x(1)+((1+gamma)*k2+alpha_L)*x(5)*k1*(x(1))^(k2)-alpha_L*(x(5)*(1-k0)+am-c_);
      (1+gamma+alpha_L)*(pb*(1-Q)+pe)*x(2)+((1+gamma)*k2+alpha_L)*x(5)*k1*(x(2))^(k2)-alpha_L*(x(5)*(1-k0)+am-c_);
