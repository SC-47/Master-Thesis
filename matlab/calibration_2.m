...
[x,fva_cal]=fsolve(@(x)myfun_2(x,pb(i),pe(i),am(i),c_(i),k0(i),k1(i),k2(i),wf_L(i),wf_H(i),tf_L(i),tf_H(i),b_L(i),b_H(i)),x0_t,options);
    bd_L_m(i)    = x(1) ;
    be_L_m(i)    = x(2) ;
...
A       = zeros(1,3);
pi      = zeros(1,3);
Z_L_m   = N_L.*tf_L_m ; %L_type labor force
Z_H_m   = N_H.*tf_H_m ; %H_type labor force
for i = 1:3
x0_t = [1.1885 0.5454];
[x,fval_A_pi]=fsolve(@(x)myfun_A_pi(x,Z_L_m(i),Z_H_m(i),wf_L(i),wf_H(i),rho),x0_t);
A(i)  = x(1) ; 
pi(i) = x(2) ;
end
